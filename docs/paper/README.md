...
The illustrated architecture is generated by [draw_architecture.py](draw_architecture.py). It exports the paper's [vector PDF](figures/pipeline.pdf), an [editable SVG](figures/pipeline.svg), and the matching GitHub overview in `docs/assets/workflow.svg` and `workflow.png`. The diagram includes schematic paired meshes, rigs, category-budget bars, following-mode curves, an extended shoe, and scoped contact checks; these drawings are not additional experimental results. Run `python draw_architecture.py` to update only this illustration, or use the full paper build to keep every output synchronized.

## Citation

```bibtex
@misc{tomTomIEE2026sbcst,
  author = {tomTomIEE},
